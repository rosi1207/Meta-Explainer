...
from aux_functions import ensure_single_channel, normalize_map, is_valid_map, evaluate_explanations
...
def intersection_operator_class_by_class(maps, weights, threshold=0.85, operator='sum'):
    ...
    Args:
        maps (list of numpy.ndarray): List of heatmaps. Shape: (n_methods, n_classes, 224, 224, 1)
    ...
    Returns:
        numpy.ndarray: Integrated heatmap per class (n_classes, 224, 224, 1), where values reflect the combined relevance.
    """
    ...
    for class_idx in range(num_classes):
        ...
        integrated_map = ensure_single_channel(integrated_map)  # Ensure shape (224, 224, 1)

        # Initialize an intersection mask that initially marks all pixels as relevant
        intersection_mask = np.ones_like(integrated_map, dtype=bool).squeeze()  # (224, 224)
...
def calculate_metric_intersection_integrated_explanations(explanations_data, y_selected, metrics, model, x_selected, threshold=0.85, operator='sum'):
    """
    ...
    for i in range(8):  # For each image
        ...
        integrated_explanation = intersection_operator_class_by_class(maps, metric_values, threshold, operator=operator)
        integrated_explanations.append(integrated_explanation)

    metric_value = evaluate_explanations(integrated_explanations, metrics, model, x_selected, y_selected)
