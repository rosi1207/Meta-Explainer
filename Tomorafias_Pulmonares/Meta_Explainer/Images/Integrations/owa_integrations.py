import numpy as np
from aux_functions import ensure_single_channel, is_valid_map, normalize_map, evaluate_explanations
...
def owa_operator_class_by_class(maps, weights, method="weighted_sum"):
    ...
    Args:
        maps (list of numpy.ndarray): List of heatmaps from different methods, with shape (n_methods, n_classes, 224, 224, 1).
    ...
    Returns:
        numpy.ndarray: Integrated heatmap per class with shape (n_classes, 224, 224, 1).
    """
...
def calculate_metric_integrated_explanations(explanations_data, y_selected, metrics, model, x_selected, method="weighted_sum"):
    """
